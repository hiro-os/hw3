...
void my_packet_handler(u_char *args, const struct pcap_pkthdr *packet_header, const u_char *packet)
{
	...
	i=ETHER_ADDR_LEN;
	printf("\tDestination address:（Mac目的のアドレス） ");
	do{
	...
	if(ntohs(eth_header->ether_type)==ETHERTYPE_IP){          //0x0800
		...
		printf("3. IPv4 source address（元のIPアドレス）: %s\tdestination address（目的のIPのアドレス）: %s\n", ipv4_source_addr, ipv4_dest_addr);
		...
		if(ipv4_header->ip_p==IPPROTO_TCP){
			...
			printf("   Source port（元のポートのアドレス）: %d\tDestination port（目的のポートのアドレス）: %d\n", source_port, dest_port);
		}
		// udp
	...
	// if the packet is IPv6
		...
		printf("3. IPv6 source address（元のIPアドレス）: %s\tdestination address（目的のIPのアドレス）: %s\n", ipv6_source_addr, ipv6_dest_addr);
